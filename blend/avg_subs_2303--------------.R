...
for (var in cols) subm_02[[var]] = 0.5*((subm_002A[[var]]*.5) + (subm_002B[[var]]*.5)) + 0.5*rfcn[[var]]
...
# Check the NoF's against yolo544
yolo[,image:= paste0(image, ".jpg")]
yolo = yolo[,(max(proba)), by="image"]
setnames(yolo, c("image", "proba"))
notNoFImages = yolo[proba>.02]$image
subm_05[!image %in% notNoFImages][["NoF"]] = 2
...
setwd("~/Dropbox/fish/sub")
  write.csv(subm_05, "avg_(boostLAG_rfcn)(boost_yolo_NoF)(resnet_box_over_0.7_full)_70_30_(avg_2_best)_2303.csv",  row.names = F)
