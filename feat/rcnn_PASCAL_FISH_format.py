...
def refresh_dir(PATH):
    ...
    for f in files:
        os.remove(f)
refresh_dir('rcnn')
refresh_dir('rcnn/VOCdevkitfish')
refresh_dir('rcnn/VOCdevkitfish/VOC2007')
refresh_dir('rcnn/VOCdevkitfish/VOC2007/Annotations')
refresh_dir('rcnn/VOCdevkitfish/VOC2007/ImageSets')
refresh_dir('rcnn/VOCdevkitfish/VOC2007/ImageSets/Main')
refresh_dir('rcnn/VOCdevkitfish/VOC2007/JPEGImages')
...
for c in crop_classes:
    ...
    for l in j: 
        ...
        if len(l["annotations"]) == 0:
            ...
            imgxmls.append(basename)
            f = open(os.path.join(anno_path, 'rcnn/VOCdevkitfish/VOC2007/Annotations/' + basename + '.xml'),'w') 
            line = "<annotation>" + '\n'
            f.write(line)
...
imgs = [imgs[i] for i in index if imgs[i] in imgxmls]
with open(os.path.join(anno_path, 'rcnn/VOCdevkitfish/VOC2007/ImageSets/Main', 'trainval.txt'),'w') as f:
...
for fish in crop_classes:
    fish_dir = TRAIN_DIR+'{}'.format(fish)
    imgs_fish += [os.path.splitext(im)[0] for im in os.listdir(fish_dir)]
trainval_fish = [im + '  1' for im in trainval if im in imgxmls]#[im+'  1' if im in imgs_fish else im+' -1' for im in trainval]
with open(os.path.join(anno_path, 'rcnn/VOCdevkitfish/VOC2007/ImageSets/Main', 'fish_trainval.txt'),'w') as f:
...
with open(os.path.join(anno_path, 'rcnn/VOCdevkitfish/VOC2007/ImageSets/Main', 'test.txt'),'w') as f:
...
for fish in FISH_CLASSES:
    ...
    for img_ in imgs:
        copy2(os.path.join(fish_dir, img_), 
              os.path.join(anno_path, 'rcnn/VOCdevkitfish/VOC2007/JPEGImages', img_))
fish_dir = TEST_DIR
