...
subm_05A  <- fread("sub/subm_full_convbig_resnet_5A.csv")
subm_05B  <- fread("sub/subm_full_convbig_resnet_5B.csv")
...
# Get the average of the large box cropped - yolo 414
subm_05 = subm_05A
cols = names(subm_05)[-1]
for (var in cols) subm_05[[var]] = (subm_05A[[var]] + subm_05B[[var]])/2
setnames(subm_05, "image_file", "image")
subm_05 = subm_05[order(image)]
rm(subm_05A, subm_05B)
...
for (var in cols) subm_034[[var]] = (subm_03[[var]]*.3) + (subm_04[[var]]*.3)  + (subm_05[[var]]*.4) 
...
# Round #1 Sub
write.csv(subm_final, paste0("sub/final-add-544yolo-addLarge.csv"), row.names = F)
