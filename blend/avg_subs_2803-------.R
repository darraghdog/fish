...
subm_05[LAG>0.4 | NoF >0.4 | OTHER>0.5 | SHARK>0.4| YFT>0.6]

# Boost the "LAG" scores. (This adds about 0.005 to score - not huge)
subm_05[LAG>0.4][["LAG"]] = 10
subm_05[NoF>0.4][["NoF"]] = 10
subm_05[OTHER>0.3][["OTHER"]] = 10
#subm_05[BET>0.4][["BET"]] = 10
#subm_05[SHARK>0.4][["SHARK"]] = 10
#subm_05[ALB>0.85][["ALB"]] = 10
#subm_05[YFT>0.6][["YFT"]] = 5
...
# same =fread("~/Dropbox/fish/test_fish_same_test.csv") # run .ipynb Clustering Images in Test Set 
...
write.csv(subm_06, "check.csv", row.names = F)
