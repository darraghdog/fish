...
subm_05A  <- fread("sub/subm_full_conv544_resnet_5A.csv")
subm_05B  <- fread("sub/subm_full_conv544_resnet_5B.csv")
subm_05C  <- fread("sub/subm_full_conv544_resnet_5B.csv")
...
# Get the average of the box cropped - yolo 544
subm_05 = subm_05A
...
# Get the weighted average of the 414 predictions
cols = names(subm_07)[-1]
for (var in cols) subm_07[[var]] = (subm_07[[var]]*.5) + (subm_05[[var]]*.5)
subm_07 = subm_07[order(image)]
...
for (var in cols) subm_012[[var]] = ((subm_01[[var]]*.25) + (subm_02[[var]]*.25)  + (subm_06[[var]]*.5))# *.7 +  rfcn[[var]]*.3
...
# Round #1 Sub
write.csv(subm_final, paste0("sub/final-add-544yolo-addVGGpseudorfcn-2x544.csv"), row.names = F)
