...
subm_002A <- fread("~/Dropbox/fish/sub/best_0303_separate_nof_classifier_mixed_with_yolo.csv")[order(image)] # VGG
...
for (var in cols) subm_02[[var]] = 0.7*((subm_002A[[var]]*.7) + (subm_002B[[var]]*.3)) + 0.3*rfcn[[var]]
...
for (var in cols) subm_04[[var]] = (subm_01[[var]]*.8) + (subm_02B[[var]]*.2) 
...
subm_05[LAG>0.4][["LAG"]] = 10
subm_05[NoF>0.4][["NoF"]] = 10
subm_05[OTHER>0.4][["OTHER"]] = 10
...
setwd("~/Dropbox/fish/sub")
  write.csv(subm_06, "similar_groups_avg_(fullwithyolo)(yolo544&yolo418)(boostNoF,LAG,OTHER)(resnet_box_over_0.7_full)_80_20_(avg_2_best)_3003.csv",  row.names = F)
