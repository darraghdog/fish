...
subm_01F <- fread("~/Dropbox/fish/sub/subm_full_convbig_resnet_5A.csv")
...
for (var in cols) subm_01[[var]] = (subm_01A[[var]]*.15) + (subm_01B[[var]]*.15) + (subm_01C[[var]]*.15) + (subm_01D[[var]]*.15) + (subm_01F[[var]]*.4)
subm_01A
subm_01B
subm_01C
subm_01D
...
table(idE)
subm_01tmp = subm_01
for (var in cols) subm_01tmp[id][[var]] = (subm_01[id][[var]]*.8) + (subm_01E[idE][[var]]*.2)
subm_01 = subm_01tmp
...
setwd("~/Dropbox/fish/sub")
  write.csv(subm_06, "similar_groups_avg_(rfcn)(yolo544&yolo418)(boostNoF,LAG,OTHER)(resnet_box_over_0.7_full)_70_30_(avg_2_best)_3103.csv",  row.names = F)
